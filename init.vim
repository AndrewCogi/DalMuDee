...
" 자동 줄바꿈 안되도록 함
set nowrap
" 가로로 창 분할 시, 새 창은 아래에 생기도록
set splitbelow
" 세로로 창 분할 시, 새 창은 오른쪽에 생기도록
set splitright
" 마우스 사용가능
set mouse=a
" 인코딩 한글깨짐 방지
...
" 현재 디렉토리를 자동으로 working dir로 설정
set autochdir
" 한글자 입력할때마다 매칭되는부분 알려주도록 함
set incsearch
" 검색결과에 하이라이트 표시
set hlsearch
" 무조건적으로 퍼버(열려있는 친구들) 보이도록 하기
set showtabline=2
" 왼쪽 숫자 padding 거리
set signcolumn=yes:1
" 버퍼저장안하고도 다른버퍼 이동가능
set hidden
" colorscheme 색상 그대로 적용되도록 함
set termguicolors
